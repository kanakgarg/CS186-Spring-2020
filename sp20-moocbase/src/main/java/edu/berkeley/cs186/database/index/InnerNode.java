...
class InnerNode extends BPlusNode {
    ...
    public void remove(DataBox key) {
        // TODO(proj2): implement
        //get(key).remove(key);
        BPlusNode r = this.findRebalance(this, null, null, null, null, key, null);
    }

    public BPlusNode findRebalance(BPlusNode thisNode, BPlusNode leftNode, BPlusNode rightNode,
                                   BPlusNode lAnchor, BPlusNode rAnchor, DataBox key, BPlusNode balanceNode) {
        BPlusNode removeNode, nextNode, nextLeft, nextRight, nextAncL, nextAncR, done;

        if (((InnerNode) thisNode).getKeys().size() < metadata.getOrder()) {
            balanceNode = null;
        ...
        nextNode = getChild(pos);

        if (thisNode.getClass() == InnerNode.class) {
            if (pos == 0) {
                nextLeft = ((InnerNode) leftNode).getChild(pos - 1);
                nextAncL = lAnchor;
            } else {
                nextLeft = ((InnerNode) thisNode).getChild(pos - 1);
                nextAncL = this;
            }

            if (pos == keys.size() - 1) {
                nextRight = ((InnerNode) rightNode).getChild(0);
                nextAncR = rAnchor;
            } else {
                nextRight = ((InnerNode) thisNode).getChild(pos + 1);
                nextAncR = this;
            }
            ...
            if (((LeafNode) thisNode).getKeys().indexOf(key) != -1) {
                ((LeafNode) thisNode).getRids().remove(keys.indexOf(key));
                ((LeafNode) thisNode).getKeys().remove(key);
                sync();
            } else {
                removeNode = null;
            }
    ...
    public BPlusNode collapseRoot(BPlusNode oldRoot) {

        if (oldRoot.getClass() == InnerNode.class) {
    ...
    public BPlusNode rebalance(BPlusNode thisNode, BPlusNode leftNode, BPlusNode rightNode, BPlusNode lAnchor, BPlusNode rAnchor) {
        InnerNode balanceNode = ((InnerNode) rightNode);
        boolean l = false;
        if (((InnerNode) leftNode).getKeys().size() > ((InnerNode) rightNode).getKeys().size()) {
            balanceNode = ((InnerNode) leftNode);
            l = true;

        }

        BPlusNode anchorNode, done;

        if (balanceNode.getKeys().size() < metadata.getOrder()) {
            if (l) {
                anchorNode = lAnchor;
            } else {
                anchorNode = rAnchor;
    ...
    public BPlusNode shift(BPlusNode thisNode, BPlusNode neighborNode, BPlusNode anchorNode) {
        ...
        while (size(neighborNode) != size(thisNode)) {
    ...
    public BPlusNode merge(BPlusNode thisNode, BPlusNode neighborNode, BPlusNode anchorNode) {
        ...
        if (thisNode.getClass() == LeafNode.class){
